...
cm_plot <- as.data.frame(cm_tbl$table) |>
  ...
  ggplot2::theme_minimal() + ggplot2::labs(title="Confusion Matrix (Aggregated)")
...
pr_plot <- ggplot2::ggplot(pr_df, aes(recall, precision)) + ggplot2::geom_line() +
  ggplot2::labs(title=sprintf("PR Curve (Lapse) — AUC=%.3f", pr$auc.integral)) + ggplot2::theme_minimal()
